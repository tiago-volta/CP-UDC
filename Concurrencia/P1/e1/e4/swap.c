...
void *swap(void *ptr)
{
    ...
    while(1) {
        ...
        // Bloquear acceso a iterations
        pthread_mutex_lock(&args->buffer->iterMutex);
        if (args->buffer->globalIter <= 0) {
            pthread_mutex_unlock(&args->buffer->iterMutex);
            break; // Salir si ya no hay iteraciones restantes
        }
        args->buffer->globalIter--; // Decrementamos el contador global

        // Si al decrementar se llegó a 0, señalamos al printer que se detenga.
        if (args->buffer->globalIter == 0) {
...
void start_threads(struct options opt)
{
    ...
    buffer.globalIter = opt.iterations;
    ...
    //Flag was stablished to true in the last swap
...
int main (int argc, char **argv)
{
    ...
    opt.iterations  = 100;
    opt.delay       = 10;
    opt.print_wait = 1; // 1 segundo en microsegundos
